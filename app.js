...
function calculateAverage(votes) {
  const validVotes = votes.filter(vote => typeof vote === 'number' && !isNaN(vote)); // Exclure les valeurs non numériques
  const sum = validVotes.reduce((total, vote) => total + vote, 0);
  return sum / validVotes.length; // Supprimer l'arrondi pour le test
}
...
module.exports = {
  ...
  displayBacklog,
  saveResults,
  getPlayerNames // Ajouter cette ligne
};
